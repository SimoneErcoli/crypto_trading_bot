...
An autonomous Python bot that runs a swing-trading strategy on Kraken with real orders,
Telegram notifications, and web-dashboard monitoring. Runs 24/7 without human intervention.
...
2. [Kraken API Key](#2-kraken-api-key)
...
3. BotFather replies with the **Bot Token** (format: `123456789:AABBCCDDee...`)
4. Copy the token into `TELEGRAM_BOT_TOKEN` in your `.env` file

**Get your Chat ID:**
1. Search for **@userinfobot** on Telegram
2. It replies with your numeric ID — copy it into `TELEGRAM_CHAT_ID`
...
4. Copy the **API Key** and **Private Key (Secret)** into your `.env` file

> **Security tip:** restrict the key to your server's IP address in Kraken's API settings.
...
# Capital and risk
CAPITALE_TOTALE=100          # euros to deploy
RISCHIO_PER_TRADE=0.015

# Mode
PAPER_TRADING=true           # true = simulation, false = live orders
STRATEGIA=conservative       # conservative | aggressive
```

> Always start with `PAPER_TRADING=true` to verify the setup before going live.
...
The bot aligns to the next 4h candle boundary (00:00, 04:00, 08:00, 12:00, 16:00, 20:00 UTC),
then repeats every 4 hours. Logs are written to `bot.log` (daily rotation, 7 days retention).

On startup it sends a Telegram confirmation with strategy, capital, and time of the next scan.
A daily report is sent at 20:00 UTC.
...
- Open positions with live prices and SL→TP2 progress bar
- Status of every asset (in position / cooldown / listening)
- Trade history with cumulative P&L chart
- Real-time log with colour-coded severity levels

Auto-refreshes every 30 seconds. No API key required to run.
...
sudo journalctl -u kraken-bot -f   # stream logs
```
...
Alternatively, create a `start.bat` file:
...
Every time a position is opened the bot places on Kraken:
- **Native Kraken stop-loss** — lives on Kraken's servers, not in the bot. Triggers automatically even if the bot is offline.
- **Take-profit limit orders** — also on Kraken, independent of the bot.

On restart the bot re-reads `positions.json`, reconnects to open orders, and resumes monitoring. No position is lost.
...
10 assets on EUR pairs, chosen for liquidity and volatility:
...
With low capital some assets may not reach Kraken's order minimum.
The bot handles this automatically: if the computed size is below the minimum but the
available balance covers it, it **scales up to the minimum** instead of skipping the trade.
...
> With €100 the bot is fully operational across all 10 assets.
> To eliminate any scaling, use a capital of €200 or more.
...
| EMA(200)  | price above the slow moving average (bullish macro trend) |
| MACD      | histogram > 0 or bullish crossover on the last candle |
| Volume    | > 1.3× 20-period average |
| ADX(14)   | > 20 (trending market, not ranging) |

### Exits

| Event | Action |
|-------|--------|
| ATR stop-loss      | entry − 2 × ATR(14), capped at −6 % of cost |
| TP1 +5%            | close 50 %, move SL to breakeven |
| TP1 → trailing stop| SL trails −3 % below the running high |
| TP2 +10%           | close 30 %, position closed |
| RSI > 72           | SELL signal → immediate close |
| Bearish MACD div.  | SELL signal → immediate close |
| Flat position > 96h| time exit, capital freed |

### Risk controls

- Max 4 open positions simultaneously
- 4-hour cooldown after every close
- 24-hour global pause after 2 consecutive stop-losses
...
Suited for trending markets with high volume.
...
| EMA(20)   | price above the more reactive moving average |
...
| Event | Action |
|-------|--------|
| ATR stop-loss      | entry − 1.5 × ATR(14), capped at −3 % of cost |
| TP1 +3%            | close 40 %, move SL to breakeven |
| TP1 → trailing stop| SL trails −2 % below the running high |
| TP2 +7%            | close 35 % |
...
- Size 1.25× the standard allocation (capped at available balance)
- 1-hour cooldown after every close
- 24-hour global pause after 3 consecutive stop-losses

> **When to use Aggressive:** clear trending markets, high ADX, rising volume.
> The tight stop (ATR × 1.5) limits losses on false breakouts; TP3 (+15 %) captures
> extended moves. Not recommended during sideways or high-uncertainty phases.
...
BUY signal (RSI + EMA + MACD + Volume + ADX + EMA200)
    ...
    └─▶ Limit buy order placed (maker price)
            │
            └─▶ Polling every 60 s (max 30 min)
                    │
                    ├─▶ [expired] ──────────────────▶ cancelled, retry on next scan
                    │
                    └─▶ [filled]
                            │
...
| File                  | Contents |
|-----------------------|----------|
| `positions.json`      | live state of all positions (entry, SL, TP, order ID) |
| `trades_history.json` | complete history of closed trades |
| `bot.log`             | daily rotating log file |
| `.pause_until`        | resume timestamp after a global pause |
...
| 4h scan               | signal for each asset (RSI, EMA, MACD, Volume, ADX, EMA200) |
| Order sent            | type, limit price, quantity, order ID |
| Position opened       | entry, SL (ATR), TP1/2/3, trailing stop, indicators |
| TP1 reached           | partial close, trailing stop activated |
| TP2 / TP3 reached     | partial/final close, total P&L, ROI |
| Trailing stop         | running high, exit price, locked-in profit |
| Stop-loss             | loss amount, consecutive SL count |
| Signal exit           | close on RSI/MACD while in position |
| Time exit             | flat position closed on timeout |
| Max positions reached | BUY ignored, limit hit |
| Pause activated       | N consecutive SL, resume time |
| API error             | attempt N/3, retry |
| Daily report          | P&L, closed trades, open positions, fees |
...
├── position_manager.py   # positions.json and trades_history.json (thread-safe)
├── risk_manager.py       # Sizing, SL/TP levels, StrategyConfig, global pause
├── telegram_notify.py    # Format and send all Telegram messages
...
├── .pause_until          # Auto-generated: global pause timestamp
├── .env                  # Credentials and configuration (do not commit)
